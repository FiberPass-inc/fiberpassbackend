...
export async function sendRecipientInviteEmail(input: RecipientInviteInput): Promise<void> {
  ...
  const body = '<p style="margin:0 0 18px;color:#d7ddeb;font-size:15px;line-height:1.7;">Hi ' + escapeHtml(input.recipientName) + ', ' + escapeHtml(input.payerName) + ' added you to a FiberPass payment. Add your CKB wallet address before the link expires so FiberPass can route the payout through Fiber Network and settle it to your wallet.</p>' +
  ...
  await sendEmail({
    ...
    html: emailShell('Payment details needed', 'Add your CKB wallet address to receive this FiberPass transfer.', body),
    headers: {
