...
export async function createAppApiKey(appId: string, walletId: string, label = 'Default key'): Promise<CreatedAppApiKeyDto> {
...
export async function authenticateAppApiKey(secret: string, routeAppId?: string): Promise<AppAuthContext> {
